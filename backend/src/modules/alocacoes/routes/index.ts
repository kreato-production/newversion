...
export function createAlocacoesRoutes(authService: AuthService, alocacoesService: AlocacoesService): FastifyPluginAsync {
  return async (app) => {
    ...
    app.post('/gravacoes/:id/alocacoes', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
      const params = request.params as { id: string };
    ...
    app.patch('/gravacoes/:id/alocacoes/:allocationId', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
    ...
    app.post('/gravacoes/:id/alocacoes/:allocationId/colaboradores', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
      const params = request.params as { id: string; allocationId: string };
    ...
    app.delete('/gravacoes/:id/alocacoes/:allocationId', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
      const params = request.params as { id: string; allocationId: string };
