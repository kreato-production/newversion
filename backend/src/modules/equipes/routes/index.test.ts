...
class InMemoryEquipesRepository implements EquipesRepository {
  ...
  async save(input: SaveEquipeInput) {
    const item: EquipeRecord = { id: input.id ?? crypto.randomUUID(), tenantId: input.tenantId, codigo: input.codigo, descricao: input.descricao, createdAt: new Date('2026-03-25T12:00:00.000Z') };
...
describe('equipes routes', () => {
  it('cria e lista equipes via API protegida', async () => {
    const authService = new AuthService(new RouteAuthRepository(), () => new Date('2026-03-25T10:00:00.000Z'));
    const equipesService = new EquipesService(new InMemoryEquipesRepository());
    const app = await buildApp({ authService, equipesService });
    ...
    const createResponse = await app.inject({
      ...
      payload: { codigo: 'EQ-1', descricao: 'Equipe 1' },
    });
    ...
    expect(listResponse.json().data).toEqual([
      expect.objectContaining({ codigo: 'EQ-1', descricao: 'Equipe 1' }),
    ]);
