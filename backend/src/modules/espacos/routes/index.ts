...
export function createEspacosRoutes(authService: AuthService, espacosService: EspacosService): FastifyPluginAsync {
  return async (app) => {
    ...
    app.post('/espacos', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
      const body = saveEspacoSchema.parse(request.body);
      return reply.status(200).send(await espacosService.save(user, body));
    });

    app.put('/espacos/:id', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
    ...
    app.delete('/espacos/:id', { preHandler: [authenticate, requireTenantAccess] }, async (request, reply) => {
      const { user } = request as AuthenticatedRequest;
      const params = request.params as { id: string };
