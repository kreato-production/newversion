...
class InMemoryProgramasRepository implements ProgramasRepository {
  ...
  async save(input: SaveProgramaInput) { const item: ProgramaRecord = { id: input.id ?? crypto.randomUUID(), tenantId: input.tenantId, codigoExterno: input.codigoExterno ?? null, nome: input.nome, descricao: input.descricao ?? null, cor: input.cor ?? null, unidadeNegocioId: input.unidadeNegocioId ?? null, unidadeNegocioNome: 'Unidade A', createdAt: new Date('2026-03-25T12:00:00.000Z') }; this.items.set(item.id, item); return item; }
