...
// Costs are accumulated inline per resource; this stub preserves call sites without double-counting
const calcularCustoGravacao = (_gravacaoId: string): number => 0;
...
const Mapas = () => {
  ...
  const recarregarDados = useCallback(async () => {
    try {
      ...
      // gravacaoRecursos is an alias for alocacoesPorGravacao, already set above
  ...
  const rhEscalas = useMemo(() => {
    ...
    recursosHumanosCadastro.forEach((rh) => {
      escalasMap[rh.id] = (rh.escalas || []) as {
        dataInicio: string;
        dataFim: string;
        diasSemana?: number[];
        horaInicio?: string;
