...
export const TabelaPrecoRecursosFisicosTab = ({
  tabelaPrecoId,
  readOnly,
  moeda = 'BRL',
}: Props) => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await repository.listRecursos(tabelaPrecoId, 'fisico');
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast({
        title: 'Erro',
        description: `Erro ao adicionar recurso físico: ${(error as Error).message}`,
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      await repository.removeRecurso(tabelaPrecoId, 'fisico', id);
      await fetchData();
    } catch (error) {
      console.error(error);
      toast({
        title: 'Erro',
        description: `Erro ao remover recurso físico: ${(error as Error).message}`,
        variant: 'destructive',
      });
    }
  };
        ...
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <Select value={selectedRecurso} onValueChange={setSelectedRecurso}>
              <SelectTrigger>
                <SelectValue placeholder="Selecione um recurso físico" />
              </SelectTrigger>
              <SelectContent>
